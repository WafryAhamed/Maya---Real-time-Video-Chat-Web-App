...
export function Room({ roomId, onLeave }: RoomProps) {
  ...
  // Simulate mock reactions from remote users occasionally
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (Math.random() > 0.7) {
        const peer =
        remoteStreams[Math.floor(Math.random() * remoteStreams.length)];
        const emoji =
  ...
  // Simulate mock poll vote from remote users
  useEffect(() => {
    if (!activePoll || !activePoll.isActive) return;
    const timer = setTimeout(() => {
      remoteStreams.forEach((peer, i) => {
        const voteTimer = setTimeout(
          () => {
            setActivePoll((prev) => {
              if (!prev || !prev.isActive) return prev;
              const alreadyVoted = prev.options.some((o) =>
              o.votes.includes(peer.user.id)
              );
              if (alreadyVoted) return prev;
              const optIdx = Math.floor(Math.random() * prev.options.length);
              return {
                ...prev,
